...
def fill_pdf(input_pdf_path, buf, data_dict):
    ...
    for page in template_pdf.pages:
        ...
        if annotations is not None:
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    annotation.update(pdfrw.PdfDict(Border=PdfArray([0, 0, 0])))
                    annotation.update(pdfrw.PdfDict(MK=PdfDict(BC=PdfArray([0, 0, 0]))))
                    if PARENT_KEY in annotation:
                        annotation = annotation[PARENT_KEY]
                    if annotation[ANNOT_FIELD_KEY]:
                        key = annotation[ANNOT_FIELD_KEY][1:-1]
...
def clone_pdf(sfx):
    ...
    for j in range(0, len(page["/Annots"])):
        ...
        if annot_get is not None:
            writer_annot.update({NameObject("/T"): createStringObject(annot_get + sfx)})
